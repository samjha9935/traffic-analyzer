...
void deleteKey(struct Node **head_ref, long long int key) 
{ 
    // Store head node 
...
void *logger(void *log){
	...
	while(30){
		...
		if(cleaner(ctime(&logtime))[12] == '0' && cleaner(ctime(&logtime))[13] == '0' && cleaner(ctime(&logtime))[15] == '0' && cleaner(ctime(&logtime))[16] == '0'){
		strcpy(strcat(name,".log"),cleaner(ctime(&logtime)));
		log = fopen(name,"a+");
		}
	fprintf(log,"%s :Status -> OK\nack: %d\nfin: %d\nrst: %d\npush: %d\nsyn: %d\nsynAck: %d\n\n",ctime(&logtime),c_ack,c_fin,c_rst,c_push,c_syn,lenSYNACK_l);
	}
}
...
int main(int argc, char **argv)
{
	...
	FILE* log;
	char name[40];
	strcpy(name,cleaner(ctime(&curtime)));
	log = fopen(strcat(name,".log"),"a+");
	char filter_exp[] = "tcp and host 192.168.102.30";		/* filter expression [3] */
	...
	sleep(1);
	pthread_t log_thread, check_thread, deleter_thread;
	pthread_create(&log_thread,NULL,logger,(void*) log);
    pthread_create(&check_thread,NULL,checker,NULL);
    pthread_create(&deleter_thread,NULL,delpkt,(void*)&args);
