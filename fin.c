...
void* wbuff_append(void *args){
	...
	while(1){
        sleep(5);
        struct Node *tp = storage_buffer;
        time_t timer;
        timer = time(NULL);
        if(semaphore == 1){
            while(storage_buffer != NULL){
                if(timer - storage_buffer->time >=300){
                    push(&write_buffer, storage_buffer->pkt_no, storage_buffer->from_ip, storage_buffer->to_ip, storage_buffer->src_port, storage_buffer->dst_port, storage_buffer->seq_no, storage_buffer->ack_no, storage_buffer->time, storage_buffer->timestamp
	, storage_buffer->FIN, storage_buffer->SYN, storage_buffer->RST, storage_buffer->PUSH, storage_buffer->ACK, storage_buffer->URG, storage_buffer->ECE, storage_buffer->CWR);
                }
...
void* writer(void *args){
    ...
    while(1){
        sleep(5);
         struct Node *tp = write_buffer;
        if(semaphore == 0){
            while(write_buffer != NULL){
				printf("\n\n Writing !!!\n--------------\n\n");
                fprintf(pktcap,"\n{\n\"Packet_no\" :%d,\n ", write_buffer->pkt_no);
		        fprintf(pktcap,"\n\"Time\" : \"%s\",\n",write_buffer->timestamp);
		        fprintf(pktcap,"\"IP\":{\n\"From\": \"%s\",\n\"To\": \"%s\"\n},\n\n", write_buffer->from_ip, write_buffer->to_ip);
		        fprintf(pktcap,"\"Ports\":{\n\"Source\": %d,\n\"Destination\": %d},\n\n\"ID\":{\n\"Sequence_no.\": %"PRIu32",\n\"Acknowledgement_no\": %"PRIu32"\n},\n\n", write_buffer->src_port, write_buffer->dst_port, write_buffer->seq_no, write_buffer->ack_no);
		        fprintf(pktcap,"\"Flags\":{\n \"FIN\": %d,\n \"SYN\": %d,\n \"RST\": %d,\n \"PUSH\": %d,\n \"ACK\": %d,\n \"URG\": %d,\n \"ECE\": %d,\n \"CWR\": %d \n}\n},\n\n\n" , write_buffer->FIN, write_buffer->SYN, write_buffer->RST, write_buffer->PUSH, write_buffer->ACK, write_buffer->URG, write_buffer->ECE, write_buffer->CWR);
                t = write_buffer->time;
                deleteKey(&write_buffer,t);
...
int main(int argc, char **argv)
{
	...
	log = fopen(name,"a+");
	char filter_exp[] = "tcp and host 192.168.0.103";		/* filter expression [3] */
